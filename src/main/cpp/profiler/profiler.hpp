...
class Profiler {
 public:
  static Profiler* create(JVM* jvm, std::chrono::nanoseconds threshold,
                          const std::string& output_path, std::chrono::nanoseconds interval);
 ...
 private:
  ...
  std::atomic<AsyncTrace*> trace_;
  std::atomic_int output_counter{0};

  Profiler(JVM* jvm, std::chrono::nanoseconds threshold, std::string output_path,
